...
const features = [
  { icon: Sparkles, title: 'Prompt Engineering', desc: 'Structured prompts for 8 content types — not a generic text box' },
  { icon: Clock, title: 'Version History', desc: 'Compare all your generations side-by-side like a Git diff for writing' },
  { icon: BarChart2, title: 'AI Improvement Engine', desc: 'Get a score and 3 specific suggestions on every piece of content' },
  { icon: Zap, title: 'Token Analytics', desc: 'Full visibility into AI usage and your content productivity' },
];
  ...
  return (
    <div style={{ minHeight:'100vh', background:'var(--bg)' }}>
      ...
      <section style={{ textAlign:'center', padding:'100px 20px 60px',
        ...
        <h1 style={{ fontFamily:'var(--font-display)', fontSize:'clamp(36px,6vw,68px)',
          fontWeight:800, lineHeight:1.1, letterSpacing:'-2px', marginBottom:20 }}>
          Generate content that<br />
          <span style={{ background:'linear-gradient(135deg, #7c6aff, #a78bfa)',
            WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent' }}>
            actually sounds human
          </span>
        </h1>
        <p style={{ fontSize:18, color:'var(--text2)', maxWidth:540,
        ...
        <p style={{ fontSize:13, color:'var(--text3)' }}>No credit card · Free forever</p>

        {/* Pills */}
        <div style={{ display:'flex', flexWrap:'wrap', justifyContent:'center',
          gap:10, maxWidth:600, margin:'40px auto 0' }}>
          {contentTypes.map(t => (
            <span key={t} style={{ padding:'6px 16px', borderRadius:100,
              border:'1px solid var(--border)', fontSize:13,
              color:'var(--text2)', background:'var(--bg2)' }}>{t}</span>
          ))}
        </div>
      </section>
      ...
      <section style={{ padding:'80px 60px', maxWidth:1100, margin:'0 auto' }}>
        <h2 style={{ fontFamily:'var(--font-display)', fontSize:32, fontWeight:700,
          textAlign:'center', marginBottom:48 }}>Why ContentForge is different</h2>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(240px,1fr))', gap:20 }}>
          {features.map(({ icon: Icon, title, desc }) => (
            <div key={title} className="card" style={{ transition:'all 0.2s' }}
              onMouseEnter={e => e.currentTarget.style.borderColor='var(--border-hover)'}
              onMouseLeave={e => e.currentTarget.style.borderColor='var(--border)'}>
              <div style={{ width:44, height:44, background:'rgba(124,106,255,0.1)',
                borderRadius:12, display:'flex', alignItems:'center',
                justifyContent:'center', marginBottom:16 }}>
                <Icon size={22} color="#7c6aff" />
              </div>
              <h3 style={{ fontFamily:'var(--font-display)', fontSize:17,
                fontWeight:700, marginBottom:8 }}>{title}</h3>
      ...
      {/* Footer */}
      <footer style={{ textAlign:'center', padding:24, color:'var(--text3)',
